...
| **ML Data Simulation** | Faker (or hand-rolled generators), pandas | Fully self-generated resource fleet for the ML dataset — no external IaC modules or datasets. See §8.3 |
...
| **Evaluator flagged use of any external dataset/IaC-module source for the ML baseline** — even an unlabeled one — as not meeting the "simulate the data" requirement. | Drop `terraform-aws-modules`/HCL parsing entirely. Both the normal baseline and the labeled anomalies are generated by our own `simulate_resources.py` + `mutate_iac.py`. See §8.3. |
...
**Features:** Fully simulated dataset — no external dataset or third-party IaC modules; both the "normal" fleet and the labeled misconfigurations are generated by our own code; train Isolation Forest (fleet anomaly/drift) + XGBoost (risk prioritization); composite 0–100 risk score per resource.
...
### 8.3 Dataset generation (corrected — fully simulated, no external dataset)

> **Evaluator constraint:** no external dataset or third-party corpus may be used anywhere in the ML pipeline — not even as an unlabeled baseline. Every resource, normal or anomalous, must be produced by code we wrote. This replaces the earlier `terraform-aws-modules` sourcing plan below.

1. **Simulated Baseline (90%):** A `simulate_resources.py` script generates the "Normal" cluster directly — no Terraform, no external modules. It builds resource JSON (S3 buckets, IAM policies, security groups, EC2 instances, etc.) in the same shape the rule engine and normalizer already expect, sampling realistic-but-randomized values (bucket ACLs, policy statements, CIDR ranges, tags, attached-policy counts) from ranges/templates we define ourselves, so the fleet looks structurally varied without tracing back to any real or downloaded source.
2. **Mutated Anomalies (10%):** A `mutate_iac.py` script takes output from the same simulator and injects structural variations (swap CIDRs to `0.0.0.0/0`, flip bucket privacy, insert wildcard IAM) — used **only for the supervised XGBoost label set**, never as the sole basis for the unsupervised model.
...
1. **Simulate:** `simulate_resources.py` generates resource JSON state from our own templates/ranges — no external dataset or IaC module source (Simulated Baseline, 90%).
2. **Mutate:** `mutate_iac.py` injects misconfigs into the simulated resources → labeled anomalies (10%) for XGBoost.
